...
{
    public class GeminiLauncherForm : Form
    {
        ...
        private async Task StartGeminiCli(string workDir)
        {
            ...
            var workingCommand = await FindWorkingGeminiCommand(workDir);
            if (workingCommand == null)
            {
                Log(IsJapanese ? "エラー: 利用可能なGemini CLIコマンドが見つかりません。" : "ERROR: No working Gemini CLI command found.");
            ...
            LaunchGeminiInNewWindow(workDir, workingCommand);
            Log(IsJapanese ? "Gemini CLIが正常に起動しました。ランチャーを終了します..." : "Gemini CLI started successfully. Closing launcher...");
        ...
        private async Task<string?> FindWorkingGeminiCommand(string workDir)
        {
            string[] geminiCommands = { "gemini.cmd", "gemini" };
            
            foreach (var cmd in geminiCommands)
            {
        ...
        private async Task<bool> CheckAndInstallGeminiCliAsync(string workDir)
        {
            Log(IsJapanese ? "\nGemini CLIを確認中..." : "\nChecking for Gemini CLI...");
            if (await IsCommandAvailableAsync("gemini.cmd", "--version", workDir))
            {
                Log(IsJapanese ? "Gemini CLIは既にインストールされています。" : "Gemini CLI is already installed.");
                return false;
            }

            return await InstallGeminiCli(workDir);
        }

        private async Task<bool> InstallGeminiCli(string workDir)
        {
            Log(IsJapanese ? "Gemini CLIがインストールされていません。npm経由でインストール中..." : "Gemini CLI is not installed. Installing via npm...");
            try
            {
                await RunProcessAsync("npm", $"install -g {GEMINI_NPM_PACKAGE}", workDir);
                Log(IsJapanese ? "Gemini CLIのインストールが完了しました。" : "Gemini CLI installation completed.");
                
                return await VerifyGeminiInstallation(workDir);
            }
            catch (Exception ex)
            {
                Log(IsJapanese ? $"Gemini CLIのインストールに失敗しました: {ex.Message}" : $"Failed to install Gemini CLI: {ex.Message}");
                Log(IsJapanese ? "Node.js/npmが正しくインストールされていることを確認して、再試行してください。" : "Please ensure Node.js/npm is properly installed and try again.");
                return false;
            }
        }

        private async Task<bool> VerifyGeminiInstallation(string workDir)
        {
            if (await IsCommandAvailableAsync("gemini.cmd", "--version", workDir))
            {
                Log(IsJapanese ? "Gemini CLIのインストールが正常に確認されました。" : "Gemini CLI installation verified successfully.");
                return true;
            }
            
            if (await IsCommandAvailableAsync("gemini", "--version", workDir))
            {
                Log(IsJapanese ? "Gemini CLIのインストールが正常に確認されました（'gemini'コマンドを使用）。" : "Gemini CLI installation verified successfully (using 'gemini' command).");
                return true;
            }
            
            Log(IsJapanese ? "警告: Gemini CLIのインストールを確認できませんでした。再起動またはPATHの確認が必要な場合があります。" : "WARNING: Gemini CLI installation could not be verified. You may need to restart or check your PATH.");
            return true; // Still recommend restart
        }
