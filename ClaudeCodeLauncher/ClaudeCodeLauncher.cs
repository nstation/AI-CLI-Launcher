...
{
    public class ClaudeCodeLauncherForm : Form
    {
        private TextBox _workDirTextBox = null!;
        private Button _browseButton = null!;
        private Button _startButton = null!;
        private TextBox _logTextBox = null!;
        private CheckBox _skipPermissionsCheckBox = null!;

        // Constants
        private const string NODE_WINGET_ID = "OpenJS.NodeJS";
        private const string CLAUDE_CODE_NPM_PACKAGE = "@anthropic-ai/claude-code";
        ...
        public ClaudeCodeLauncherForm(bool autoStart = false, string? savedWorkDir = null)
        {
        ...
        private void InitializeComponent()
        {
            ...
            _skipPermissionsCheckBox = new CheckBox
            {
        ...
        private async Task<(bool ShouldRestart, bool NodeInstalled, bool GeminiInstalled)> CheckAndInstallDependencies(string workDir)
        {
            ...
            var claudeCodeInstalled = await CheckAndInstallClaudeCodeAsync(workDir);
            
            return (nodeInstalled || claudeCodeInstalled, nodeInstalled, claudeCodeInstalled);
        }
        ...
        private async Task StartClaudeCode(string workDir)
        {
            Log(IsJapanese ? "\nClaude Codeを起動中..." : "\nStarting Claude Code...");
            LogCurrentPath();

            var workingCommand = await FindWorkingClaudeCodeCommand(workDir);
            if (workingCommand == null)
            {
                Log(IsJapanese ? "エラー: 利用可能なClaude Codeコマンドが見つかりません。" : "ERROR: No working Claude Code command found.");
            ...
            LaunchClaudeCodeInNewWindow(workDir, workingCommand);
            Log(IsJapanese ? "Claude Codeが正常に起動しました。ランチャーを終了します..." : "Claude Code started successfully. Closing launcher...");
        ...
        private async Task<string?> FindWorkingClaudeCodeCommand(string workDir)
        {
            string[] claudeCodeCommands = { "claude.cmd", "claude" };
            
            foreach (var cmd in claudeCodeCommands)
            {
        ...
        private async Task<bool> CheckAndInstallClaudeCodeAsync(string workDir)
        {
            Log(IsJapanese ? "\nClaude Codeを確認中..." : "\nChecking for Claude Code...");
            if (await IsCommandAvailableAsync("claude.cmd", "--version", workDir))
            {
                Log(IsJapanese ? "Claude Codeは既にインストールされています。" : "Claude Code is already installed.");
                return false;
            }

            return await InstallClaudeCode(workDir);
        }

        private async Task<bool> InstallClaudeCode(string workDir)
        {
            Log(IsJapanese ? "Claude Codeがインストールされていません。npm経由でインストール中..." : "Claude Code is not installed. Installing via npm...");
            try
            {
                await RunProcessAsync("npm", $"install -g {CLAUDE_CODE_NPM_PACKAGE}", workDir);
                Log(IsJapanese ? "Claude Codeのインストールが完了しました。" : "Claude Code installation completed.");
                
                return await VerifyClaudeCodeInstallation(workDir);
            }
            catch (Exception ex)
            {
                Log(IsJapanese ? $"Claude Codeのインストールに失敗しました: {ex.Message}" : $"Failed to install Claude Code: {ex.Message}");
                Log(IsJapanese ? "Node.js/npmが正しくインストールされていることを確認して、再試行してください。" : "Please ensure Node.js/npm is properly installed and try again.");
                return false;
            }
        }

        private async Task<bool> VerifyClaudeCodeInstallation(string workDir)
        {
            if (await IsCommandAvailableAsync("claude.cmd", "--version", workDir))
            {
                Log(IsJapanese ? "Claude Codeのインストールが正常に確認されました。" : "Claude Code installation verified successfully.");
                return true;
            }
            
            if (await IsCommandAvailableAsync("claude", "--version", workDir))
            {
                Log(IsJapanese ? "Claude Codeのインストールが正常に確認されました（'claude'コマンドを使用）。" : "Claude Code installation verified successfully (using 'claude' command).");
                return true;
            }
            
            Log(IsJapanese ? "警告: Claude Codeのインストールを確認できませんでした。再起動またはPATHの確認が必要な場合があります。" : "WARNING: Claude Code installation could not be verified. You may need to restart or check your PATH.");
            return true; // Still recommend restart
        }
