...
namespace CodexUninstaller
{
    public class UninstallerForm : Form
    {
        ...
        private const string CODEX_COMMAND_NAME = "codex";
        private const string CODEX_CONFIG_FOLDER = ".codex";
        ...
        {
            Log(IsJapanese ? "Codex CLIの存在確認中..." : "Checking Codex CLI installation...");
            var isInstalled = await CheckPackageInstalledAsync(CODEX_NPM_PACKAGE);
            ...
            else
            {
                ...
                var exitCode = await RunProcessAsync("npm.cmd", $"uninstall -g {CODEX_NPM_PACKAGE}");
            ...
            await RemoveCommandFilesAsync(CODEX_COMMAND_NAME);
            await RemoveConfigFolderAsync(CODEX_CONFIG_FOLDER);
        }
