...
namespace ClaudeCodeUninstaller
{
    public class ClaudeCodeUninstallerForm : Form
    {
        ...
        private const string CLAUDE_COMMAND_NAME = "claude";
        private const string CLAUDE_CONFIG_FOLDER = ".claude";
        ...
        {
            Log(IsJapanese ? "Claude Codeの存在確認中..." : "Checking Claude Code installation...");
            var isInstalled = await CheckPackageInstalledAsync(CLAUDE_CODE_NPM_PACKAGE);
            ...
            else
            {
                ...
                var exitCode = await RunProcessAsync("npm.cmd", $"uninstall -g {CLAUDE_CODE_NPM_PACKAGE}");
            ...
            await RemoveCommandFilesAsync(CLAUDE_COMMAND_NAME);
            await RemoveConfigFolderAsync(CLAUDE_CONFIG_FOLDER);
        }
